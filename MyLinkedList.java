...
public boolean delete(E item)
  ...
  public String toString() {
	...
	public String extract(E number)
	{
		MyLinkedList<E> listLess = new MyLinkedList<>();
		MyLinkedList<E> listGreater = new MyLinkedList<>();
		Node<E> ptr = head;
		
		while(ptr.next != null)
		{
			if(((Comparable)ptr.element).compareTo(number) < 0)
			{
				listLess.append(ptr.element);
			}
			...
			if(((Comparable)ptr.element).compareTo(number) > 0)
			{
				listGreater.append(ptr.element);
			}
			ptr = ptr.next;
		}
		
		return  "\n\nList after extract: "+listGreater.toString()+"\n\nExtracted: "+listLess.toString() ;
		//return listLess;
